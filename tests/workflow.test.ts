...
describe("provenance (W3C PROV)", () => {
  it("derives an Entity/Activity/Agent graph with actedOnBehalfOf for a human-gated run", async () => {
    const { provenanceOf } = await import("../src/workflow/provenance.js");
    const spec = workflow({
      name: "release",
      steps: [{ id: "draft", ask: "write" }, { id: "ok", approve: "Ship?" }, { id: "ship", use: "deploy.run", with: {} }],
    });
    const first = await startRun(spec, {}, deps(() => answer("copy")));
    const done = await resumeRun(first.id, { approved: true, approver: "cfo@acme" }, spec, deps(() => answer("copy")));
    const g = provenanceOf(done);
    // the draft output is an entity generated by the draft activity, attributed to the workflow agent
