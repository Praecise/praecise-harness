...
export function apiModule(manifest: PackageManifest): string {
  ...
  // `callPublished` is framework machinery rather than public API, so it comes
  // from the unstable subpath. This module is regenerated by `buildPackage` on
  // every build, so it tracks whatever version of praecise built it.
  return `import { App } from "@praecise/harness";
import { callPublished } from "@praecise/harness/internal";
