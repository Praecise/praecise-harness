...
`SELVES_URL` and the rest) and the `praecise-person` header.
...
- **Breaking (shape of the public API), ahead of 1.0.** The package root now
  ...
  `@praecise/harness/internal` subpath (see *Added*), so every current import keeps
  ...
  Moved to `@praecise/harness/internal`: `findCycle`, `loadProject`, `resolveKnows`,
...
- The `api.js` module generated by `buildPackage` now imports `callPublished`
  from `@praecise/harness/internal` rather than from the root. It is regenerated on every
  build, so it tracks whichever version of praecise built it.

### Added

- **`@praecise/harness/internal`**, a second entry point holding everything demoted
...
- **`API.md`**, the written-down public API, grouped by task — authoring,
  ...
  what `@praecise/harness/internal` does and does not promise.
